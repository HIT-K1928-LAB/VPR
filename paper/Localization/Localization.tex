...
\begin{abstract}
Visual localization in large-scale scenes hinges on the quality of global retrieval. This paper presents otgaussvlad, an optimal-transport-based Gaussian residual aggregator for place recognition. The proposed layer projects backbone tokens into a latent descriptor space, scores them with diagonal-Gaussian energy or normalized cosine similarity, and solves a Sinkhorn transport problem with a learnable dustbin and token-mass prediction. On top of the OT branch, we further introduce a self-attentive Gaussian query codebook and an OT-anchored refinement path that predicts bounded corrections for the image-side Gaussian moments without changing the descriptor dimensionality. Transport mass is retained at the cluster level and used to build mean and standard-deviation residuals with respect to learnable Gaussian prototypes. The concatenated 2KD-dimensional descriptor is directly $\ell_2$-normalized without an additional projection head. Experiments on Pittsburgh 30k and Aachen Day-Night demonstrate faster convergence and improved retrieval and localization performance over NetVLAD within a standard hierarchical localization pipeline.
\end{abstract}
...
(2) We design a self-attentive Gaussian query codebook and an OT-anchored refinement branch, which use bounded residual gates to adapt prototype parameters and image-side Gaussian moments while remaining close to the base OT solution at initialization.

(3) We derive a transport-aware Gaussian residual descriptor that preserves cluster mass, models both mean and scale deviations, and admits a diagonal-Gaussian Wasserstein interpretation of the resulting cluster residuals.

(4) Extensive experiments show that the proposed aggregator improves retrieval accuracy and training stability over NetVLAD while remaining fully compatible with standard hierarchical localization back ends.
...
Let \(X \in \mathbb{R}^{C \times H \times W}\) denote the backbone output. After the two-layer point-wise adapter and LayerNorm, the feature map is flattened into \(N = HW\) local tokens \(z_n \in \mathbb{R}^D\). In the default configuration, \(C = 768\), \(D = 64\), and \(K = 64\), resulting in a raw descriptor dimension of \(2KD = 8192\). No projection head is applied after aggregation.

For each cluster \(k \in \{1,\dots,K\}\), the layer parameterizes a diagonal Gaussian prototype
\[
p_k(z) = \mathcal{N}(z; \mu_k, \operatorname{diag}(\sigma_k^2)),
\]
where \(\mu_k \in \mathbb{R}^D\), \(\sigma_k \in \mathbb{R}_+^D\), and the mixture prior is
\[
\pi_k = \frac{e^{\alpha_k}}{\sum_{j=1}^K e^{\alpha_j}}.
\]
The default GMM score is the corresponding log-density up to a cluster-independent constant:
\begin{equation}
s_{kn}
= \log \pi_k
...
\label{eq:ot_score}
\end{equation}
For completeness, the attention ablation uses normalized prototypes and tokens, \(\bar{\mu}_k = \mu_k / \|\mu_k\|_2\) and \(\bar{z}_n = z_n / \|z_n\|_2\), with score \(s^{\mathrm{att}}_{kn} = \tau \langle \bar{\mu}_k, \bar{z}_n \rangle\), where \(\tau = \operatorname{softplus}(a) + \epsilon\).

When \texttt{token\_mass\_mode} is set to \texttt{learned}, a lightweight token saliency head produces logits \(q_n\) and masses
\[
\nu_n = \frac{\exp(\eta q_n)}{\sum_{m=1}^N \exp(\eta q_m)},
\qquad \eta = \operatorname{softplus}(\lambda) + \epsilon.
\]
In the uniform case, \(\nu_n = 1/N\). The target mass vector is \(n = \nu / \|\nu\|_1\).

To suppress uninformative tokens, the score matrix is augmented with a learnable dustbin row and the following entropy-regularized transport problem is solved:
\[
A^\star = \arg\max_{A \in \mathcal{U}(m,n)} \langle \tilde{S}, A \rangle + \mathcal{H}(A),
\]
...
\]
Here \(\delta \in (0,1)\) is the dustbin mass and \(\tilde{S}\) denotes the score matrix after appending the dustbin row. The implemented log-space Sinkhorn iterations compute the associated dual scalings.
...
\end{proposition}
\begin{proof}
The feasible set \(\mathcal{U}(m,n)\) is non-empty, closed, and bounded, hence compact. The map \(A \mapsto \langle \tilde{S}, A \rangle\) is linear, and \(\mathcal{H}(A)\) is strictly concave on the positive orthant. Therefore the objective is strictly concave over a compact convex set, which yields existence and uniqueness of the maximizer.
\end{proof}

Let \(A\) be the real-cluster block of \(A^\star\). For each cluster \(k\), define the transported mass
\[
m_k = \sum_{n=1}^N A_{kn},
\qquad
\gamma_{kn} = \frac{A_{kn}}{\max(m_k,\epsilon)}.
\]
If \(m_k > 0\), then \(\sum_n \gamma_{kn} = 1\), so \(\gamma_{k\cdot}\) defines a probability distribution over the tokens assigned to cluster \(k\). The transport-induced empirical moments are then
\begin{align}
...
\]
so that the final cluster residual becomes \(\tilde{r}_k = w_k r_k\). This normalization is anchored at the uniform reference mass: if \(m_k = (1-\delta)/K\), then \(w_k = 1\). The final global descriptor is obtained by concatenating \(\tilde{r}_1,\ldots,\tilde{r}_K\) and applying \(\ell_2\) normalization:
\[
...
\subsection{Self-Attentive Gaussian Query Codebook}
The implementation in \texttt{ot\_gaussvlad\_gaussian\_query\_codebook.py} makes the Gaussian codebook slightly adaptive by replacing the fixed prototype table with a learnable query set. Let \(u_k \in \mathbb{R}^{C_g}\) denote the \(k\)-th Gaussian query slot and \(Q(\cdot)\) be a self-attentive block with optional residual attention and LayerNorm. The slot representation is
\begin{equation}
 h_k = Q(u_k),
\end{equation}
followed by two bounded residual heads:
\begin{align}
\Delta \mu_k &= s_\mu \tanh(W_\mu h_k), \\
\Delta \ell_k &= s_\sigma \tanh(W_\ell h_k),
\end{align}
where \(\ell_k = \log \sigma_k\), \(s_\mu\), and \(s_\sigma\) are learnable scales initialized to small positive values through an inverse-sigmoid parameterization. The adaptive Gaussian codebook is then
\begin{equation}
\tilde{\mu}_k = \mu_k + \Delta \mu_k,\qquad
\tilde{\ell}_k = \ell_k + \Delta \ell_k.
\end{equation}
Because the residual heads are zero-initialized and the scales start small, the module is close to the original prototype table at the beginning of training and can only deviate when the data supports it.

\subsection{OT-Anchored Moment Refinement}
The stronger variant in \texttt{ot\_gaussvlad\_gaussian\_query\_codebook\_ot\_anchored\_refine.py} keeps the OT assignment branch intact, but refines the image-side Gaussian statistics after transport. Given the transported assignments \(A\), the OT branch first produces
\begin{equation}
\hat{\mu}^{\mathrm{ot}}_k = \sum_n \gamma_{kn} z_n,\qquad
\hat{\sigma}^{\mathrm{ot}}_k = \sqrt{\sum_n \gamma_{kn} z_n^2 - (\hat{\mu}^{\mathrm{ot}}_k)^2},
\end{equation}
where \(\gamma_{kn}\) is the row-normalized assignment.

The refiner is parameterized as a small transformer-style block. Its input query is the concatenation \([\mu_k,\log\sigma_k]\), which is first normalized by a \(2D\)-dimensional LayerNorm and then projected by a two-layer MLP \(\mathrm{Linear}(2D \to C)\!\rightarrow\!\mathrm{GELU}\!\rightarrow\!\mathrm{Dropout}\!\rightarrow\!\mathrm{Linear}(C \to C)\). This produces a cluster query in the latent refinement space. The local tokens are similarly normalized by a \(D\)-dimensional LayerNorm and projected with a linear map into the same latent dimension so that query and memory share the attention embedding space.

The core interaction is a batch-first multi-head cross-attention module,
\begin{equation}
\mathrm{Attn}(Q,M) = \mathrm{MHA}(Q,M,M),
\end{equation}
where the query sequence is the Gaussian codebook and the key/value sequence is the projected local-token memory. The attention output is added back to the query branch through a residual connection, followed by LayerNorm. If enabled, a position-wise feed-forward block is applied on top of the attention features:
\begin{equation}
\mathrm{FFN}(x) = W_2\,\mathrm{Dropout}(\mathrm{GELU}(W_1 x)),
\end{equation}
with residual addition after a second normalization. This makes the refiner a lightweight transformer encoder rather than a single attention layer.

From the refined latent features, two independent linear heads predict the residual corrections for mean and log-standard-deviation:
\begin{align}
\Delta \mu_k &= W_\mu h_k + b_\mu, \\
\Delta \ell_k &= W_\ell h_k + b_\ell.
\end{align}
Both heads are explicitly zero-initialized, so the refiner begins at the identity mapping and initially leaves the OT moments unchanged. Optionally, a \(\tanh\) nonlinearity is applied to bound the raw residual before scaling. The residual magnitude is then multiplied by learnable gates
\begin{equation}
\alpha_\mu = s_{\max}\,\sigma(\rho_\mu),\qquad
\alpha_\sigma = s_{\max}\,\sigma(\rho_\sigma),
\end{equation}
where \(s_{\max}\) is the maximum residual scale and \(\sigma(\cdot)\) is the sigmoid function. The logit parameters \(\rho_\mu\) and \(\rho_\sigma\) are initialized using an inverse-sigmoid map so that the effective scales start at small positive values (e.g., 0.05 by default). This guarantees that the correction branch is conservative at the beginning of training and can only deviate gradually from the OT estimate.

The final moment estimates are anchored to the OT statistics rather than replacing them:
\begin{equation}
\hat{\mu}_k = \hat{\mu}^{\mathrm{ot}}_k + \Delta \mu_k,\qquad
\hat{\ell}_k = \log(\hat{\sigma}^{\mathrm{ot}}_k) + \Delta \ell_k.
\end{equation}
Finally, the image-side scale is recovered by exponentiation and clamped to a small positive constant for numerical stability, matching the implementation of \(\hat{\sigma}_k = \exp(\hat{\ell}_k)\). This design is important in practice: the refinement branch starts almost inactive, so optimization initially follows the stable OT solution and then learns image-side corrections only when they reduce retrieval loss.
