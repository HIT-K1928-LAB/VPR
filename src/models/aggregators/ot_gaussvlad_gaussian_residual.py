"""
OT-GaussVLAD with stronger Gaussian residual parameterizations.

The assignment and aggregation backbone follows OT-GaussVLAD, while the final
cluster residual can be switched among:

    raw:          [mu_hat - mu, sigma_hat - sigma]
    log_sigma:    [mu_hat - mu, log(sigma_hat) - log(sigma)]
    standardized: [(mu_hat - mu) / sigma, log(sigma_hat / sigma)]

The standardized form is the default because it is scale-aware and closer to a
Gaussian natural residual.
"""
...
class OTGaussVLADGaussianResidualLayer(nn.Module):
    """
    OT-GaussVLAD with configurable diagonal Gaussian residuals.
    ...
    assignment:
        Sinkhorn + dustbin
    aggregation:
        raw, log-sigma, or standardized Gaussian residual
    """
    ...
    def __init__(
        ...
        if residual_mode not in {"raw", "log_sigma", "standardized"}:
            raise ValueError("residual_mode must be one of: raw, log_sigma, standardized.")
    ...
    def _gaussian_residual(self, mu_hat: torch.Tensor, sigma_hat: torch.Tensor):
        cluster_mu = self.mu.unsqueeze(0)
        cluster_log_sigma = self.log_sigma.unsqueeze(0)
        cluster_sigma = self._sigma().unsqueeze(0)

        if self.residual_mode == "raw":
            mean_shift = mu_hat - cluster_mu
            sigma_shift = sigma_hat - cluster_sigma
        elif self.residual_mode == "log_sigma":
            mean_shift = mu_hat - cluster_mu
            sigma_shift = torch.log(sigma_hat.clamp_min(self.eps)) - cluster_log_sigma
        else:
    ...
    def forward(self, x):
        ...
        mean_shift, sigma_shift = self._gaussian_residual(mu_hat, sigma_hat)
