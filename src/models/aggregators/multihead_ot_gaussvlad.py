"""
Multi-head OT-GaussVLAD.

This BoQ-inspired variant runs multiple OT transport heads in parallel over the
same local tokens. Each head produces a Gaussian residual descriptor, the head
descriptors are concatenated, and a learnable projection compresses them back to
a fixed descriptor dimension.
...
class MultiHeadOTGaussVLADLayer(nn.Module):
    """
    Parallel multi-head OT-GaussVLAD with concat-projection fusion.
    ...
    def __init__(
        ...
        self.raw_dim_per_head = self.num_clusters * 2 * self.cluster_dim
        ...
        proj_dropout = nn.Dropout(projection_dropout) if projection_dropout > 0 else nn.Identity()
    ...
    def _head_descriptor(
        ...
        return torch.cat([mean_shift, sigma_shift], dim=-1).reshape(local.size(0), -1)
    ...
    def forward(self, x):
        ...
        concat_descriptor = torch.cat(head_descriptors, dim=1)
        descriptor = self.projection(concat_descriptor)
